...
/**
 * In deze klas staat alle informatie van een dank e-mail.
 * De informatie staat in de klas zelf of wordt opgehaald vanaf het template in het register.
 *
 * @author dennis
 */
public class DankMail implements MailInhoud {
     ...
     */
    public DankMail(MailController mc) {
    ...
    public void setDefaultAanhef() {
        aanhef = prefs.get("dankMail_aanhef", "");
        if (aanhef.isEmpty()) {
    ...
    public void setDefaultBody() {
        this.body = prefs.get("dankMail_body", "");
        if (body.isEmpty()) {
    ...
    public void setDefaultSubject() {
        this.subject = prefs.get("dankMail_onderwerp", "");
        if (subject.isEmpty()) {
